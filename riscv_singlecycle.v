...
module riscv_singlecycle (
    ...
    // ---------------- Next PC logic ----------------
    // Branch: taken only if branch=1 AND the comparison the ALU just
    // did (a SUB) came out how the funct3 wants it.
    //   funct3=000 (BEQ): taken if zero=1        (rs1 == rs2)
    //   funct3=001 (BNE): taken if zero=0        (rs1 != rs2)
    //   others (BLT/BGE/etc.) intentionally left as a later addition --
    //   flagged in the decisions log as a known scope limit for now.
